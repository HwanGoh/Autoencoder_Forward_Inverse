...
#                             Training Properties                             #
###############################################################################
def optimize(hyperp, run_options, file_paths, NN, parameter_and_state_obs_train, parameter_and_state_obs_val, parameter_and_state_obs_test, parameter_dimension, num_batches_train):
    #=== Optimizer ===#
...
###############################################################################
    ...
    def train_step(batch_parameter_train, batch_state_obs_train):
        with tf.GradientTape() as tape:
            ...
            batch_loss_train_encoder = loss_encoder(batch_state_pred_train, batch_state_obs_train, hyperp.penalty)
    ...
    def val_step(batch_parameter_val, batch_state_obs_val):
        ...
        batch_loss_val_encoder = loss_encoder(batch_state_pred_val, batch_state_obs_val, hyperp.penalty)
    ...
    def test_step(batch_parameter_test, batch_state_obs_test):
        ...
        batch_loss_test_encoder = loss_encoder(batch_state_pred_test, batch_state_obs_test, hyperp.penalty)
...
############################################################################### 
    ...
    for epoch in range(hyperp.num_epochs):
        ...
        print('Train Loss: Full: %.3e, Parameter: %.3e, State: %.3e' %(mean_loss_train.result(), mean_loss_train_autoencoder.result(), mean_loss_train_encoder.result()))
