...
def KLD_diagonal_post_cov(post_mean, log_post_var, prior_mean, prior_cov_inv, log_det_prior_cov, latent_dimension):    
    trace_prior_cov_inv_times_cov_post = tf.reduce_sum(tf.multiply(tf.linalg.diag_part(prior_cov_inv), np.exp(log_post_var)), axis=1)
    ...
    log_det_prior_cov_divide_det_cov_post = log_det_prior_cov - tf.math.reduce_sum(log_post_var, axis=1) # Going to form determinant by subtracting log(1) one by one to avoid numerical error
