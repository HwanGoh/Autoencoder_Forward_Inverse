...
def KLD_diagonal_post_cov(post_mean, log_post_var, prior_mean, prior_cov_inv, log_det_prior_cov, latent_dimension):    
    ...
    prior_weighted_prior_mean_minus_post_mean = tf.reduce_sum(tf.multiply(tf.transpose(prior_mean - post_mean), tf.linalg.matmul(prior_cov_inv, tf.transpose(prior_mean - post_mean))), axis = 0)              
    log_det_prior_cov_divide_det_cov_post = log_det_prior_cov - tf.math.reduce_sum(log_post_var, axis=1)
