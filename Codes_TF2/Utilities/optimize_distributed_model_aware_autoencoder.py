...
#                             Training Properties                             #
###############################################################################
def optimize_distributed(dist_strategy, GLOBAL_BATCH_SIZE, hyperp, run_options, file_paths, NN, parameter_and_state_obs_train, parameter_and_state_obs_val, parameter_and_state_obs_test, parameter_dimension, num_batches_train):
    ...
    with dist_strategy.scope():    
        ...
        mean_relative_error_parameter_decoder = tf.keras.metrics.Mean()
    ...
    storage_array_relative_error_parameter_decoder = np.array([])
    ...
    with dist_strategy.scope():
        ...
        def train_step(batch_parameter_train, batch_state_obs_train):
            with tf.GradientTape() as tape:
                ...
                unscaled_replica_batch_loss_train_encoder = loss_encoder(batch_state_pred_train, batch_state_obs_train, hyperp.penalty)
                unscaled_replica_batch_loss_train_batch = unscaled_replica_batch_loss_train_autoencoder + unscaled_replica_batch_loss_train_encoder
            ...
            mean_loss_train_encoder(unscaled_replica_batch_loss_train_batch)
        ...
        def val_step(batch_parameter_val, batch_state_obs_val):
            ...
            unscaled_replica_batch_loss_val_encoder = loss_encoder(batch_state_pred_val, batch_state_obs_val, hyperp.penalty)
        ...
        def test_step(batch_parameter_test, batch_state_obs_test):
            ...
            batch_parameter_pred_test_decoder = NN.decoder(batch_state_obs_test)
            ...
            unscaled_replica_batch_loss_test_encoder = loss_encoder(batch_state_pred_test, batch_state_obs_test, hyperp.penalty)
            ...
            mean_relative_error_parameter_decoder(relative_error(batch_parameter_pred_test_decoder, batch_parameter_test))
...
############################################################################### 
    ...
    for epoch in range(hyperp.num_epochs):
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar('relative_error_parameter_decoder', mean_relative_error_parameter_decoder.result(), step=epoch)
        ...
        storage_array_relative_error_parameter_decoder = np.append(storage_array_relative_error_parameter_decoder, mean_relative_error_parameter_decoder.result())
        ...
        print('Train Loss: Full: %.3e, Parameter: %.3e, State: %.3e' %(mean_loss_train, mean_loss_train_autoencoder.result(), mean_loss_train_encoder.result()))
        print('Val Loss: Full: %.3e, Parameter: %.3e, State: %.3e' %(mean_loss_val.result(), mean_loss_val_autoencoder.result(), mean_loss_val_encoder.result()))
        print('Test Loss: Full: %.3e, Parameter: %.3e, State: %.3e' %(mean_loss_test.result(), mean_loss_test_autoencoder.result(), mean_loss_test_encoder.result()))
        print('Rel Errors: AE: %.3e, Inverse: %.3e, Forward: %.3e\n' %(mean_relative_error_parameter_autoencoder.result(), mean_relative_error_parameter_decoder.result(), mean_relative_error_state_obs.result()))
        start_time_epoch = time.time()   
        ...
        mean_relative_error_parameter_decoder.reset_states()
