...
#                             Training Properties                             #
###############################################################################
def optimize(hyperp, run_options, file_paths, NN, obs_indices, loss_autoencoder, loss_forward_problem, loss_model_augmented, relative_error, parameter_and_state_obs_train, parameter_and_state_obs_val, parameter_and_state_obs_test, parameter_dimension, num_batches_train):
...
###############################################################################
    ...
    def train_step(batch_parameter_train, batch_state_obs_train):
        with tf.GradientTape() as tape:
            ...
            batch_loss_train_model_augmented = loss_model_augmented(hyperp, run_options, V, solver, obs_indices, batch_state_obs_train, batch_parameter_pred_train_AE, hyperp.penalty_aug)
            batch_loss_train = batch_loss_train_autoencoder + batch_loss_train_forward_problem + batch_loss_train_model_augmented
    ...
    def val_step(batch_parameter_val, batch_state_obs_val):
        ...
        batch_loss_val_model_augmented = loss_model_augmented(hyperp, run_options, V, solver, obs_indices, batch_state_obs_val, batch_parameter_pred_val_AE, hyperp.penalty_aug)
        batch_loss_val = batch_loss_val_autoencoder + batch_loss_val_forward_problem + batch_loss_val_model_augmented
    ...
    def test_step(batch_parameter_test, batch_state_obs_test):
        ...
        batch_loss_test_model_augmented = loss_model_augmented(hyperp, run_options, V, solver, obs_indices, batch_state_obs_test, batch_parameter_pred_test_AE, hyperp.penalty_aug)
        batch_loss_test = batch_loss_test_autoencoder + batch_loss_test_forward_problem + batch_loss_test_model_augmented
...
############################################################################### 
    ...
    for epoch in range(hyperp.num_epochs):
        ...
        print('Train Loss: Full: %.3e, Parameter: %.3e, State: %.3e, State Aug: %.3e' %(mean_loss_train.result(), mean_loss_train_autoencoder.result(), mean_loss_train_forward_problem.result(), mean_loss_train_model_augmented.result()))
        print('Val Loss: Full: %.3e, Parameter: %.3e, State: %.3e, State Aug: %.3e' %(mean_loss_val.result(), mean_loss_val_autoencoder.result(), mean_loss_val_forward_problem.result(), mean_loss_val_model_augmented.result()))
        print('Test Loss: Full: %.3e, Parameter: %.3e, State: %.3e, State Aug: %.3e' %(mean_loss_test.result(), mean_loss_test_autoencoder.result(), mean_loss_test_forward_problem.result(), mean_loss_test_model_augmented.result()))
