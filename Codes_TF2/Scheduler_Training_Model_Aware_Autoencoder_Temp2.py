...
if __name__ == '__main__':
    ...
    if rank == 0: # This is the master processes' action 
        ...
        while True:
            ...
            proc = subprocess.Popen(['./Training_Driver_Model_Aware_Autoencoder_Temp.py', f'{data.data_type}', f'{data.num_hidden_layers}', f'{data.truncation_layer}', f'{data.num_hidden_nodes}', f'{data.activation}', f'{data.penalty_encoder:.4f}', f'{data.penalty_decoder:.4f}', f'{data.penalty_prior:.4f}', f'{data.batch_size}', f'{data.num_epochs}',  f'{data.gpu}'])
            #proc = subprocess.Popen(['./Training_Driver_Reversed_Model_Aware_Autoencoder_Temp.py', f'{data.data_type}', f'{data.num_hidden_layers}', f'{data.truncation_layer}', f'{data.num_hidden_nodes}', f'{data.activation}', f'{data.penalty_encoder:.4f}', f'{data.penalty_decoder:.4f}', f'{data.penalty_prior:.4f}', f'{data.batch_size}', f'{data.num_epochs}',  f'{data.gpu}'])
