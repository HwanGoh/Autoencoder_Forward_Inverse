...
###############################################################################
    ...
    def test_step(batch_data_test, batch_latent_test):
        ...
        mean_relative_error_latent_encoder(relative_error(tf.math.exp(batch_post_mean_test),
            batch_latent_test))
...
###############################################################################
    ...
    for epoch in range(hyperp.num_epochs):
        ...
        storage_array_relative_gradient_norm =\
                np.append(storage_array_relative_gradient_norm,
        ...
        print('Train Loss: Full: %.3e, AE: %.3e, Encoder: %.3e'\
                %(mean_loss_train.result(),
                    mean_loss_train_autoencoder.result(), mean_loss_train_encoder.result()))
        print('Val Loss: Full: %.3e, AE: %.3e, Encoder: %.3e'\
                %(mean_loss_val.result(),
                    mean_loss_val_autoencoder.result(), mean_loss_val_encoder.result()))
        print('Test Loss: Full: %.3e, AE: %.3e, Encoder: %.3e'\
                %(mean_loss_test.result(),
                    mean_loss_test_autoencoder.result(), mean_loss_test_encoder.result()))
        ...
        if epoch %100 ==0:
            ...
            metrics_dict['relative_error_data_autoencoder'] =\
                    storage_array_relative_error_data_autoencoder
            metrics_dict['relative_error_latent'] = storage_array_relative_error_latent_encoder
