...
def KLD_diagonal_post_cov(post_mean, log_post_var, prior_mean, prior_cov_inv,
    ...
    trace_prior_cov_inv_times_cov_post = tf.reduce_sum(
            tf.multiply(tf.linalg.diag_part(prior_cov_inv), tf.math.exp(log_post_var)), axis=1)
    prior_weighted_prior_mean_minus_post_mean = tf.reduce_sum(tf.multiply(tf.transpose(
