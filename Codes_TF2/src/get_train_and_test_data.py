...
#                                 All Data                                    #
###############################################################################
def load_train_and_test_data(run_options, file_paths,
    ...
    if run_options.add_noise == 1:
        if load_data_train_flag == 1:
            ...
            output_train +=\
                    run_options.noise_level*output_max*np.random.normal(0, 1, output_train.shape)
        output_max = np.max(output_test)
        output_test +=\
                run_options.noise_level*output_max*np.random.normal(0, 1, output_test.shape)
