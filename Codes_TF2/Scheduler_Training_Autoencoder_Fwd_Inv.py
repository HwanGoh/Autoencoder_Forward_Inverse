...
if __name__ == '__main__':
    ...
    if rank == 0: # This is the master processes' action 
        ...
        hyperp.truncation_layer  = [2] # Indexing includes input and output layer with input layer indexed by 0
        hyperp.num_hidden_nodes  = [50, 100, 500, 1000]
        hyperp.penalty           = [50]
        ...
        for scenario_values in permutations_list: 
            ...
            setattr(hyperp_scenario, 'gpu', '0') # Set gpu number as hyperp attribute so that MPI can pass it around with the other hyperparameters
        ...
        while True:
            ...
            proc = subprocess.Popen(['./Training_Driver_Autoencoder_Fwd_Inv.py', f'{data.data_type}', f'{data.num_hidden_layers}', f'{data.truncation_layer}', f'{data.num_hidden_nodes}', f'{data.penalty:.4f}', f'{data.batch_size}', f'{data.num_epochs}',  f'{data.gpuz}'])
