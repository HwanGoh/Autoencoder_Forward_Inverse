...
def add_noise(run_options, output_train, output_test, load_data_train_flag = 0):

    np.random.seed(run_options.random_seed)
    dampening_scalar = 0.001

    #=== Add Noise ===#
    noisy_obs = np.random.choice(
            range(0, output_train.shape[1]), run_options.num_noisy_obs , replace=False)
    non_noisy_obs = np.setdiff1d(range(0, run_options.num_obs_points), noisy_obs)
    if load_data_train_flag == 1:
        ...
        output_train += run_options.noise_level*output_max*noise
    output_max = np.max(output_test)
    noise = np.random.normal(0, 1, output_test.shape)
    noise[:, non_noisy_obs] = dampening_scalar*noise[:, non_noisy_obs]
    output_test += run_options.noise_level*output_max*noise
